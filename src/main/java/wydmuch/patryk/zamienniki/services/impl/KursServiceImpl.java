...
public class KursServiceImpl implements KursService {
    ...
    public List<KursDto> getKursySearch(String trybStudiow,
        ...
        List<Przedmiot> przedmioty = mapIt(plany, PlanStudiow::getPrzedmioty);
        List<Kurs> kursy = mapIt(przedmioty, p -> filterKursy(p.getKursy(), formaZaliczenia, formyZajec, ects));
    ...
    private <T, R> List<R> mapIt(List<T> old, Function<T, List<R>> function) {
        return old.stream()
                .map(function)
                .flatMap(List::stream)
                .collect(Collectors.toList());
